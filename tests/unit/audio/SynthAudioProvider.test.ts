...
const makeFilter = () => ({
  ...
  Q: { value: 1 }, connect: vi.fn(), disconnect: vi.fn(),
})

const makeDelay = () => ({ delayTime: { value: 0 }, connect: vi.fn(), disconnect: vi.fn() })

const makeBufferSource = () => ({
  buffer: null as null, loop: false,
  connect: vi.fn(), disconnect: vi.fn(), start: vi.fn(), stop: vi.fn(),
})
...
class MockAudioContext {
  ...
  createBuffer = vi.fn((_c: number, size: number) => ({ getChannelData: () => new Float32Array(size) }))
...
describe('SynthAudioProvider', () => {
  ...
  beforeEach(() => { provider = new SynthAudioProvider(); vi.clearAllMocks() })
  ...
  it('calling unlock() twice is idempotent', async () => {
    await provider.unlock(); await provider.unlock()
    expect(provider.isUnlocked()).toBe(true)
  })
  ...
  it('play() after unlock does not throw for all known sounds', async () => {
    await provider.unlock()
    const sounds = [
      'button.hover', 'button.click', 'insight.gained', 'insight.banked', 'insight.sealed',
      'dialogue.advance', 'location.moved', 'player.died', 'loop.started', 'lighthouse.lit', 'ending.reached',
    ]
    for (const id of sounds) { expect(() => provider.play(id)).not.toThrow() }
  })
  ...
  it('stop() after play does not throw', async () => {
    await provider.unlock(); provider.play('button.hover')
    expect(() => provider.stop('button.hover')).not.toThrow()
  })
  ...
  it('setThreatLevel() does not throw for any value', async () => {
    await provider.unlock()
    for (const v of [0, 0.5, 1, -0.5, 1.5]) {
      expect(() => provider.setThreatLevel(v)).not.toThrow()
    }
  })
})
